...
class ExtraMethods {
  String getWhatsAppUrl(String number, [String msg = '']) {
    return 'https://wa.me/91$number?text=${msg.replaceAll('&', 'and')}';
  }

  void makeCall(String number) {
    if (number.startsWith('+91')) {
      launchInBrowser("tel:$number", bypass: true);
    } else {
      launchInBrowser("tel:+91$number", bypass: true);
    }
  }
  ...
  static Future<void> launchInBrowser(String url, {bool bypass = false}) async {
    if (await canLaunchUrl(Uri.parse(url)) || bypass) {
      await launchUrl(
        Uri.parse(url),
